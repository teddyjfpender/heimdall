...
class StarknetTransactionTestHelper:
    ...
    def validate_transaction_signature(response: Dict[str, Any]) -> None:
        ...
        assert response.get("success", False), f"Transaction signing failed: {response.get('error')}"
...
class TestStarknetTransactionFlowE2E:
    ...
    def test_single_user_invoke_transaction_complete_flow(self, aws_mock_fixtures):
        ...
        transaction_payload = StarknetTransactionTestHelper.create_invoke_transaction_payload()
    ...
    def test_multiuser_transaction_flow_with_key_derivation(self, aws_mock_fixtures):
        ...
        assert len(set(signatures)) == len(signatures), "Signatures should be unique for different users"
    ...
    def test_account_info_request_flow(self, aws_mock_fixtures):
        ...
        with patch('application.starknet.enclave.multiuser_server.process_account_info_request') as mock_process:
            mock_response = {
                "username": user_session["user_id"],
                "key_index": 0,
                "account_address": "0x01a4bd3c888c8bb6c5b5bb6d8d5c5e5e5f5f5f5f5f5f5f5f5f5f5f5f5f5f5f5",
            ...
            from application.starknet.enclave.multiuser_server import process_account_info_request
            response = process_account_info_request(request_payload)
            
            # Validate response
            assert response.get("success", False), f"Account info request failed: {response.get('error')}"
            assert response["username"] == user_session["user_id"]
...
class TestStarknetTransactionTypes:
    ...
    def test_declare_transaction_signing(self, aws_mock_fixtures):
        ...
        transaction_payload = StarknetTransactionTestHelper.create_declare_transaction_payload()
    ...
    def test_deploy_account_transaction_signing(self, aws_mock_fixtures):
        ...
        transaction_payload = StarknetTransactionTestHelper.create_deploy_account_transaction_payload()
...
class TestMultiUserConcurrentSigning:
    ...
    def test_concurrent_user_signing(self, aws_mock_fixtures):
        ...
        for i in range(num_users):
            user_session = aws_mock_fixtures.create_test_user_session(f"concurrent_user_{i}")
            users.append(user_session)
        ...
        with ThreadPoolExecutor(max_workers=num_users) as executor:
            futures = [executor.submit(sign_transaction_for_user, i) for i in range(num_users)]
    ...
    def test_concurrent_same_user_different_keys(self, aws_mock_fixtures):
        ...
        def sign_with_key_index(key_index: int) -> Dict[str, Any]:
            ...
            transaction_payload = StarknetTransactionTestHelper.create_invoke_transaction_payload(
                nonce=key_index,
                calldata=[0x3000 + key_index, 0x4000 + key_index]
            )
    ...
    def test_high_concurrency_stress_test(self, aws_mock_fixtures):
        ...
        for i in range(num_concurrent_requests):
            user_session = aws_mock_fixtures.create_test_user_session(f"stress_user_{i}")
            users.append(user_session)
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(stress_sign_transaction, i) for i in range(num_concurrent_requests)]
            responses = [future.result() for future in as_completed(futures)]
