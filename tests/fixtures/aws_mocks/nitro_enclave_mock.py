...
class MockAttestationDocument:
    ...
    def _generate_certificate(self) -> str:
        ...
        cert_data = {
            ...
            "not_after": (
                self.timestamp.replace(year=self.timestamp.year + 1)
            ).isoformat(),
        }
    ...
    def to_cbor(self) -> bytes:
        ...
        document = {
            ...
            "user_data": (
                base64.b64encode(self.user_data).decode() if self.user_data else None
            ),
...
class MockNitroEnclaveService:
    ...
    def terminate_enclave(self, enclave_id: str) -> Dict[str, Any]:
        ...
        return {"EnclaveID": enclave_id, "State": "TERMINATED"}
    ...
    def verify_attestation_document(
        self, attestation_doc: bytes, expected_pcrs: Optional[Dict[int, str]] = None
    ) -> Dict[str, Any]:
    ...
    def get_vsock_connection(
        self, cid: int, port: int
    ) -> Optional[MockVSockConnection]:
    ...
    def validate_enclave_environment(self) -> Dict[str, Any]:
        ...
        validation_results = {"valid": True, "checks": {}, "warnings": []}
    ...
    def get_enclave_metrics(self, enclave_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "uptime_seconds": int(
                time.time() - self.enclaves[enclave_id]["created_at"].timestamp()
            ),
        }

    def simulate_kmstool_call(
        self,
        operation: str,
...
def create_nitro_enclave_mock(region: str = "us-east-1") -> MockNitroEnclaveService:
    ...
    starknet_enclave = mock_service.create_enclave(
        "/app/starknet_enclave.eif", cpu_count=4, memory_mib=1024, debug_mode=True
    )
