...
class TestPerformanceBenchmarks:
    ...
    def test_key_derivation_performance_scaling(self, test_master_seed):
        ...
        assert performance_ratio < 5.0, f"Performance degradation too severe: {performance_ratio}"
    ...
    def test_multiple_key_derivation_performance(self, test_master_seed):
        ...
        for count, perf in performance_results.items():
            assert perf > 20, f"Multi-key performance too slow for {count} keys: {perf} keys/sec"
...
class TestConcurrencyAndThreadSafety:
    ...
    def test_concurrent_user_validation_timing(self):
        ...
        print(f"Concurrent validation timing - Mean: {mean_time*1000:.2f}ms, "
...
class TestEdgeCasesAndLimits:
    ...
    def test_hkdf_maximum_output_length(self):
        ...
        assert generation_time < 1.0, f"Max length HKDF too slow: {generation_time:.3f}s"
        
        print(f"Max HKDF output ({max_length} bytes) generated in {generation_time*1000:.1f}ms")
    ...
    def test_hkdf_with_large_inputs(self):
        ...
        generation_time = end_time - start_time
        assert generation_time < 0.1, f"Large input HKDF too slow: {generation_time:.3f}s"
    ...
    def test_key_derivation_with_extreme_usernames(self):
        ...
        print(f"Extreme usernames - Max time: {max_time*1000:.1f}ms, "
    ...
    def test_key_derivation_with_high_indices(self):
        ...
        for key_index in high_indices:
            start_time = time.perf_counter()
            private_key, attempt = derive_user_private_key(master_seed, username, key_index)
            end_time = time.perf_counter()
    ...
    def test_key_manager_with_many_users(self):
        ...
        print(f"Generated {num_users} keys in {total_time:.1f}s ({keys_per_second:.1f} keys/sec)")
        
        # Performance should be reasonable
        assert keys_per_second > 100, f"Mass key generation too slow: {keys_per_second} keys/sec"
...
class TestRobustnessAndReliability:
    ...
    def test_error_recovery_in_key_derivation(self):
        ...
        for seed, username, expected_error in error_conditions:
            from key_derivation import InvalidUserNameError
    ...
    def test_statistical_properties_validation(self):
        ...
        degrees_freedom = 255
        assert chi_square < degrees_freedom * 3, f"Poor byte distribution: chi-square = {chi_square}"
        ...
        print(f"Statistical validation passed - Chi-square: {chi_square:.1f}, "
...
class TestStressTestsAndLoadTesting:
    ...
    def test_sustained_load_key_derivation(self):
        ...
        while time.perf_counter() - start_time < duration_seconds:
            ...
            if keys_generated % 1000 == 0:
                ...
                print(f"Sustained load: {keys_generated} keys in {elapsed:.1f}s ({rate:.1f} keys/sec)")
        
        end_time = time.perf_counter()
        actual_duration = end_time - start_time
        final_rate = keys_generated / actual_duration
        
        print(f"Sustained load completed: {keys_generated} keys in {actual_duration:.1f}s "
              f"({final_rate:.1f} keys/sec)")
        
        # Should maintain reasonable performance under sustained load
        assert final_rate > 50, f"Sustained load performance too low: {final_rate} keys/sec"
    ...
    def test_high_concurrency_stress(self):
        ...
        def stress_worker(worker_id: int) -> Dict[str, any]:
            ...
            try:
                for i in range(operations_per_thread):
                    ...
                    other_username = f"stress_user_{(worker_id+1) % num_threads}_{i:03d}"
        ...
        assert total_keys == expected_keys, f"Missing keys: {expected_keys - total_keys}"
        assert total_validations == expected_validations, f"Missing validations: {expected_validations - total_validations}"
        assert total_errors == 0, f"Unexpected errors: {total_errors}"
        
        # Worker times should be reasonably balanced
        time_ratio = max_worker_time / min_worker_time if min_worker_time > 0 else float('inf')
