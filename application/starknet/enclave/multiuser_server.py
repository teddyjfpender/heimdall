...
def process_single_user_request(payload_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        signer = StarkCurveSigner(
            account_address=contract_address_int, key_pair=key_pair, chain_id=chain_id
        )
        ...
        # Create account
        account = Account(
            address=contract_address_int, client=client, signer=signer, chain=chain_id
        )
        ...
        signed_transaction = account.sign_invoke_v3_sync(
            calls=[call], l1_resource_bounds=l1_resource_bounds, nonce=nonce
        )
...
def process_multiuser_request(payload_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Create account
        account = Account(
            address=account_address_int, client=client, signer=signer, chain=chain_id
        )
