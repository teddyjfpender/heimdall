...
if [[ ! -d ./app/server ]]; then
  ...
  cd ./app/server
  cat <<'EOF' >>build_starknet_signing_server_enclave.sh
#!/usr/bin/bash
...
docker pull ${__STARKNET_SIGNING_SERVER_IMAGE_URI__}
docker pull ${__STARKNET_SIGNING_ENCLAVE_IMAGE_URI__}

nitro-cli build-enclave --docker-uri ${__STARKNET_SIGNING_ENCLAVE_IMAGE_URI__} --output-file starknet_signing_server.eif

EOF
  chmod +x build_starknet_signing_server_enclave.sh
  cd ../..
  chown -R ec2-user:ec2-user ./app

  sudo -H -u ec2-user bash -c "cd /home/ec2-user/app/server && ./build_starknet_signing_server_enclave.sh"
fi

if [[ ! -f /etc/systemd/system/nitro-starknet-signing-server.service ]]; then
  ...
  cat <<'EOF' >>/etc/systemd/system/nitro-starknet-signing-server.service
[Unit]
Description=Nitro Enclaves Starknet Signing Server
...
[Service]
Type=simple
ExecStart=/home/ec2-user/app/starknet_watchdog.py
Restart=always
#RestartSec=5
...
EOF
  ...
  cat <<EOF >>/home/ec2-user/app/starknet_watchdog.py
#!/usr/bin/env python3
...
def nitro_cli_run_call():
    subprocess_args = [
        ...
        "--cpu-count", "2",
        "--memory", "4320",
        "--eif-path", "/home/ec2-user/app/server/starknet_signing_server.eif",
        "--enclave-cid", "16"
    ]
...
def main():
    ...
    while nitro_cli_describe_call("starknet_signing_server"):
...
EOF
  ...
  chmod +x /home/ec2-user/app/starknet_watchdog.py

fi

# start and register the nitro starknet signing server service for autostart
systemctl enable --now nitro-starknet-signing-server.service
...
docker run -d --restart unless-stopped --security-opt seccomp=unconfined --name starknet_http_server -v /etc/pki/tls/certs/:/etc/pki/tls/certs/ -p 443:443 ${__STARKNET_SIGNING_SERVER_IMAGE_URI__}
--//--